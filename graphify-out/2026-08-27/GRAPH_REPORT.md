...
## Corpus Check
- 141 files · ~80,703 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1357 nodes · 2710 edges · 98 communities (85 shown, 13 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 406 edges (avg confidence: 0.9)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `337a9a92`
...
Cohesion: 0.27
Nodes (12): _get_sessionmaker(), batch_reprocess_candidate_stream(), Any, post, StreamingResponse, reprocess_candidate(), reprocess_candidate_stream(), upload_resume() (+4 more)
...
Cohesion: 0.07
Nodes (53): batch_delete_candidates(), delete_candidate(), get_candidate(), get_candidate_file(), get_candidate_insight(), get_candidate_timeline(), list_candidates(), get (+45 more)
...
Cohesion: 0.12
Nodes (25): _acquire_cas_ref(), _bind_release_on_commit(), _build_audit_criteria(), _emit(), _has_committed_reference(), IntakeResult, _is_dummy_profile(), _load_existing_identifiers() (+17 more)
...
Cohesion: 0.21
Nodes (11): parse_docx(), Path, DOCX parser using python-docx. Public interface: parse_docx(path: Path) ->…, Extract text and metadata from a Word (.docx) file. Extraction order: 1. All…, _parse_raw_text(), _make_docx(), Path, Tests for ingestion/parsers/docx_parser.py Seam under test: parse_docx(path:… (+3 more)
...
Cohesion: 0.17
Nodes (14): BaseSettings, Settings, concurrency_env(), make_engine(), Any, fixture, Regression: parallel ingest_file must not fail with 'database is locked'.…, Same pragma set as config.database.get_engine but short busy_timeout. (+6 more)
...
Cohesion: 0.16
Nodes (21): build_job_ad_distill_prompt(), Job-ad parsing prompt for the given (already truncated) advertisement text., _ai_distill(), _ai_distill_ollama(), _ai_distill_openrouter(), _build_summary(), _coerce_recipe(), _extract_title_phrase() (+13 more)
...
Cohesion: 0.18
Nodes (16): stream_openrouter_generate(), _call_ollama(), _call_openrouter(), classify_document_llm(), Call OpenRouter API for inference. Never calls paid models., Use local LLM to classify if document is a resume., Call local Ollama for inference., get_llm_provider() (+8 more)
...
Cohesion: 0.24
Nodes (9): build_fact_extraction_prompt(), build_match_insight_prompt(), Match-rationale prompt for the given resume text, search query, and optional…, Full fact-extraction prompt for the given (already truncated) resume text., Guard tests: prompts moved into candidate_intelligence_platform.prompts must…, test_fact_extraction_prompt_is_byte_identical(), test_match_insight_prompt_is_byte_identical(), test_match_insight_prompt_tolerates_braces_in_inputs() (+1 more)
...
### Community 90 - "index_writer.py"
Cohesion: 0.33
Nodes (4): _match_section_header(), Section-aware text chunker with context injection. Public interface:…, Return the canonical section name if the line is a section header., Shared data model for all ingestion parsers.

### Community 91 - "isolate_llm_environment"
Cohesion: 0.40
...
## Knowledge Gaps
- **136 isolated node(s):** `path`, `fs`, `candidate-intelligence-platform`, `SentinelModel`, `$schema` (+131 more)
