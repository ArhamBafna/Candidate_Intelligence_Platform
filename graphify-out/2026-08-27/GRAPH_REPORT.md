...
## Corpus Check
- 139 files · ~79,972 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1327 nodes · 2618 edges · 95 communities (84 shown, 11 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 395 edges (avg confidence: 0.9)
...
2. `Settings` - 63 edges
3. `ingest_file()` - 54 edges
4. `IntakeStatus` - 33 edges
5. `IntakeSource` - 33 edges
6. `ResumeVersion` - 33 edges
7. `TimelineMode` - 31 edges
8. `CASManager` - 29 edges
9. `search_candidates()` - 27 edges
...
Cohesion: 0.11
Nodes (39): _get_sessionmaker(), batch_delete_candidates(), batch_reprocess_candidate_stream(), delete_candidate(), get_candidate(), get_candidate_file(), get_candidate_insight(), get_candidate_timeline() (+31 more)
...
### Community 2 - "db_models.py"
Cohesion: 0.21
Nodes (13): DeclarativeBase, delete_candidate_and_indices(), Any, Session, Deletes a candidate and cascades deletions to all related records and indices., Base, CandidateClaim, CandidateTimelineEvent (+5 more)
...
### Community 4 - "test_state_machine.py"
Cohesion: 0.26
Nodes (12): patch, update_candidate_status(), CandidateStateMachine, InvalidStateTransition, Transitions the candidate to a new status and logs the event., TransitionContext, Exception, db_session() (+4 more)
...
Cohesion: 0.11
Nodes (21): Any, Session, Logs a new event in the candidate's timeline., Retrieves all timeline events for a candidate, ordered by creation date…, Event-sourced logger for candidate timeline events., TimelineLedger, _bind_release_on_commit(), _emit() (+13 more)
...
Cohesion: 0.17
Nodes (25): _ai_explanation_warning(), _build_search_query(), _build_structured_filter_suffix(), _format_search_results(), _hydrate_candidates(), perform_search(), perform_search_stream(), _prepare_query() (+17 more)
...
Cohesion: 0.14
Nodes (26): get_llm_provider(), Effective provider. API key is IGNORED unless provider=openrouter., _ai_distill(), _ai_distill_ollama(), _ai_distill_openrouter(), build_search_dsl(), _build_summary(), _coerce_recipe() (+18 more)
...
Cohesion: 0.16
Nodes (14): client(), db_engine(), db_session(), isolate_test_environment(), mock_heavy_models(), mock_vector_db(), fixture, Provides an isolated SQLite file-based database engine configured with WAL mode… (+6 more)
...
Cohesion: 0.14
Nodes (13): 1. Resume Intake Pipeline Unified (`COMPLETED`), 2. Search DSL Seam & Soft Filters (`COMPLETED`), 3.1 Problem, 3.2 Target Refactor: `StorageIndexWriter` Module, 3. Database Housekeeping Knowledge Scattered (`OPEN`), 4.1 Problem, 4.2 Target Refactor, 4. Hybrid Extractor Internals & spaCy Seam (`OPEN`) (+5 more)
...
Cohesion: 0.22
Nodes (12): stream_openrouter_generate(), RuntimeError, _call_ollama(), _call_openrouter(), Call OpenRouter API for inference. Never calls paid models., Call local Ollama for inference., is_openrouter_model_free(), mark_openrouter_model_paid() (+4 more)
...
Cohesion: 0.20
Nodes (12): build_fact_extraction_prompt(), build_job_ad_distill_prompt(), build_match_insight_prompt(), Job-ad parsing prompt for the given (already truncated) advertisement text., Full fact-extraction prompt for the given (already truncated) resume text., Match-rationale prompt for the given resume text, search query, and optional…, Guard tests: prompts moved into candidate_intelligence_platform.prompts must…, test_fact_extraction_prompt_is_byte_identical() (+4 more)
...
### Community 94 - "isolate_llm_environment"
Cohesion: 0.40
