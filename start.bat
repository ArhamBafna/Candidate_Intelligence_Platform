...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if (-not (Test-Path '.\ui\node_modules')) { Push-Location '.\ui'; npm install; Pop-Location } else { Write-Host 'UI packages ready.' -ForegroundColor Green };" ^
  "Write-Host '`n[3/4] Starting FastAPI backend on http://127.0.0.1:8000...' -ForegroundColor Yellow;" ^
  "Start-Process powershell -ArgumentList '-NoExit', '-Command', ('Set-Location \"' + $curr + '\"; Write-Host \"--- FastAPI Backend (Port 8000) ---\" -ForegroundColor Cyan; uv run uvicorn api.main:app --host 127.0.0.1 --port 8000 --reload');" ^
  "Write-Host '`n[4/4] Starting Vite Frontend on http://localhost:5173...' -ForegroundColor Yellow;" ^
  "Start-Process powershell -ArgumentList '-NoExit', '-Command', ('Set-Location \"' + $curr + '\ui\"; Write-Host \"--- Vite Frontend (Port 5173) ---\" -ForegroundColor Cyan; npm run dev');" ^
  "Write-Host '`nWaiting for servers to start...' -ForegroundColor Gray;" ^
  "for ($i = 0; $i -lt 15; $i++) { try { $res = Invoke-WebRequest -Uri 'http://127.0.0.1:8000/health' -UseBasicParsing -TimeoutSec 2 -ErrorAction SilentlyContinue; if ($res.StatusCode -eq 200) { break } } catch {}; Start-Sleep -Seconds 1 };" ^
